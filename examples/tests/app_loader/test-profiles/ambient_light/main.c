...
// #define SAMPLE_INTERVAL_MS 17000  // 15 seconds
...
do {
  ...
  libtock_alarm_repeating_every_ms(AMBIENT_INTERVAL_MS, lux_cb, (void*)1, &t1);
  ...
  while (1) {
    ...
    if (lux_ready){
      lux_ready = false;
      int lux;
      int _ret2 = libtocksync_ambient_light_read_intensity(&lux);
      ...
      _ot_buf[1] = lux;
      _ack_received = false; 
      ipc_notify_service(_ot_service);
      yield_for(&_ack_received);
    }
    // int temp_c_raw;
